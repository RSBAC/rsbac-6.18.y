			 ...
			 enum mnt_tree_flags_t flags)
{
...
#ifdef CONFIG_RSBAC
	rsbac_pr_debug(aef, "do_move_mount() [do_mount() [sys_mount()]]: calling ADF for UMOUNT on old DIR\n");
	rsbac_target_id.dir.device = old_path->dentry->d_sb->s_dev;
	rsbac_target_id.dir.inode  = old_path->dentry->d_inode->i_ino;
	rsbac_target_id.dir.dentry_p = old_path->dentry;
	rsbac_attribute_value.dummy = 0;
	if (!rsbac_adf_request(R_UMOUNT,
				...
				rsbac_attribute_value))
		return -EPERM;
	rsbac_pr_debug(aef, "do_move_mount() [do_mount() [sys_mount()]]: calling ADF for MOUNT on new DIR\n");
...
#ifdef CONFIG_RSBAC
	if (likely(mp->mp->m_dentry && mp->mp->m_dentry->d_sb && mp->mp->m_dentry->d_inode)) {
		rsbac_pr_debug(aef, "[do_add_mount() [sys_mount()]]: calling ADF for DIR\n");
		rsbac_target_id.dir.device = mp->mp->m_dentry->d_sb->s_dev;
		rsbac_target_id.dir.inode  = mp->mp->m_dentry->d_inode->i_ino;
		rsbac_target_id.dir.dentry_p = mp->mp->m_dentry;
		rsbac_attribute_value.dummy = 0;
		if (!rsbac_adf_request(R_MOUNT,
					task_pid(current),
					T_DIR,
